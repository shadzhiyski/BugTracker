namespace BugTracker.Tests
{
    ...
    [TestClass]
    class BugCommentsIntegrationTests
    {
        [TestMethod]
        public void GetBugComments_ListBugComments_ShouldListBugCommentsCorrectly()
        {
            ...
            var httpResponse = TestingEngine.HttpClient
                .GetAsync("/api/bugs/" + submittedBug.Id + "/comments").Result;
            ...
            Assert.IsTrue(
                commentsFromService[0].DateCreated - DateTime.Now < TimeSpan.FromMinutes(1));
            ...
            Assert.IsTrue(
                commentsFromService[1].DateCreated - DateTime.Now < TimeSpan.FromMinutes(1));
        }
    }
}
